(() => {
  ...
  function createLightbox() {
    ...
    lb.id = "imgLightbox";
    lb.style.cssText = "position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(0,0,0,.92);display:none;align-items:center;justify-content:center;z-index:99999;cursor:zoom-out;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);padding:20px;box-sizing:border-box;";
    ...
    // Close on Escape is handled in keyboard handler
    return lb;
  }
  ...
  /* ============ POPOVERS ============ */
  function openPop(id) { const p = $(id); if (p) { p.classList.add("show"); /* Trap focus on mobile */ if (isMobile()) document.body.style.overflow = 'hidden'; } }
  function closePop(id) { const p = $(id); if (p) { p.classList.remove("show"); } restoreBodyScroll(); }
  function restoreBodyScroll() {
    /* Only restore if no popover is open */
    const anyOpen = ["accountPop", "settingsPop"].some(id => { const p = $(id); return p && p.classList.contains("show"); });
  ...
  document.addEventListener("click", e => { if (e.target.closest(".pill") && window.innerWidth <= 980) closeDrawer(); });
  ...
  const cache = new Map();
  const MAX_CACHE = 40;
  async function fetchPage(t, signal) {
  ...
  function render(a, badge, kind) {
    ...
    if ($("extract")) {
      $("extract").innerHTML = (a.extract || "<p>No content available.</p>") +
        `<div class="bottom-next-wrap"><button class="bottom-next-btn" id="bottomNextBtn"><span>DISCOVER NEXT FIND</span> ➔</button></div>`;
      const bnb = $("bottomNextBtn");
      if (bnb) bnb.onclick = () => roll();
    }
  ...
  /* ============ DISCOVERY ============ */
  async function fetchCategoryMembers(key, signal) {
    const cands = CATEGORY_CANDIDATES[key] || [key.charAt(0).toUpperCase() + key.slice(1)];
    const opts = signal ? { signal } : {};
    for (const c of cands) {
      try {
        const json = await (await fetch(`${API}?action=query&list=categorymembers&cmtitle=Category:${encodeURIComponent(c)}&cmlimit=100&cmtype=page&format=json&origin=*`, opts)).json();
  ...
  const FILTH_TOASTS = ["🔞 get freaky, scholar", "💦 wikipedia's dirty laundry, served hot", "🍑 knowledge has never been this wet", "😈 the library is horny tonight", "🫦 read it and weep, you degenerate", "🔞 get freaky, scholar", "💦 wikipedia's dirty laundry, served hot", "🍑 knowledge has never been this wet", "😈 the library is horny tonight", "🫦 read it and weep, you degenerate", " sex ed > abstinence, fight me", "🩺 your body, your data, your right", "🌈 gender is a spectrum, not a suggestion", "⚖️ consent isn't complicated, people", " trafficking isn't a kink, it's a crime"];
  ...
  async function roll() {
    ...
    try {
      if (currentCat === "all") {
        const d = await (await fetch("https://en.wikipedia.org/api/rest_v1/page/random/summary", { signal })).json();
        render(await fetchPage(d.title, signal), "✨ RANDOM FIND", "rand");
      } else if (currentCat === "adult") {
        const pickKey = ADULT_CATS[Math.floor(Math.random() * ADULT_CATS.length)];
        const m = await fetchCategoryMembers(pickKey, signal);
        if (m) {
          render(await fetchPage(m[Math.floor(Math.random() * m.length)].title, signal), "🔞 FILTH MODE", "adult");
        } else {
          render(await fetchPage(ADULT_POOL[Math.floor(Math.random() * ADULT_POOL.length)], signal), "🔞 FILTH MODE", "adult");
        }
        toast(FILTH_TOASTS[Math.floor(Math.random() * FILTH_TOASTS.length)]);
      } else {
        const key = currentCat.toLowerCase();
        const m = await fetchCategoryMembers(key, signal);
        if (m) {
          render(await fetchPage(m[Math.floor(Math.random() * m.length)].title, signal), currentChannel.toUpperCase(), "cat");
        } else if (FALLBACK_POOLS[key]) {
          const pool = FALLBACK_POOLS[key];
          render(await fetchPage(pool[Math.floor(Math.random() * pool.length)], signal), currentChannel.toUpperCase(), "cat");
        } else throw new Error("empty");
      }
    } catch (e) {
      if (e.name === 'AbortError') { busy = false; return; }
      showError("Could not fetch article. Press Next to retry.");
    }
    busy = false;
  }
  async function loadByTitle(t, badge, kind) {
    ...
    const signal = getSignal();
    try { render(await fetchPage(t, signal), badge, kind || "rand"); }
    catch (e) {
      if (e.name === 'AbortError') { busy = false; return; }
      showError("Article unreachable.");
    }
    busy = false;
  }
  if ($("trickRandom")) $("trickRandom").addEventListener("click", () => roll());
  if ($("nextBtn")) $("nextBtn").addEventListener("click", () => roll());
  if ($("trickWeird")) $("trickWeird").addEventListener("click", async () => { if (busy) return; busy = true; showLoader(); beep(500); const signal = getSignal(); try { render(await fetchPage(WEIRD[Math.floor(Math.random() * WEIRD.length)], signal), "🤪 CERTIFIED WEIRD", "weird"); } catch (e) { if (e.name !== 'AbortError') showError("Weirdness unavailable."); } busy = false; });
  if ($("trickMystery")) $("trickMystery").addEventListener("click", async () => { if (busy) return; busy = true; showLoader(); beep(400); const signal = getSignal(); try { render(await fetchPage(MYSTERIES[Math.floor(Math.random() * MYSTERIES.length)], signal), "🔮 UNSOLVED MYSTERY", "mystery"); } catch (e) { if (e.name !== 'AbortError') showError("Mystery vanished."); } busy = false; });
  if ($("trickTrail")) $("trickTrail").addEventListener("click", async () => { if (busy) return; busy = true; showLoader(); beep(700); const signal = getSignal(); try { const d = await (await fetch("https://en.wikipedia.org/api/rest_v1/page/random/summary", { signal })).json(); const full = await fetchPage(d.title, signal); render({ ...full, extract: '<p style="text-align:center;"><i>Getting to Philosophy: click the first link — you will almost always land on Philosophy.</i></p>' + full.extract }, "🧭 PHILOSOPHY TRAIL", "trail"); } catch (e) { if (e.name !== 'AbortError') showError("Trail closed."); } busy = false; });
  if ($("trickPotd")) $("trickPotd").addEventListener("click", async () => {
    ...
    try {
      ...
      const res = await fetch(`https://en.wikipedia.org/api/rest_v1/feed/featured/${yyyy}/${mm}/${dd}`, { signal });
      ...
      render({ title: titleText, extract: imgUrl ? `<div style="text-align:center;margin-bottom:15px;"><img src="${imgUrl}" alt="${escapeAttr(titleText)}" loading="lazy"></div><p style="text-align:center;">${descText}</p>` : `<p style="text-align:center;">${descText}</p>`, url: "https://en.wikipedia.org/wiki/Wikipedia:Picture_of_the_day" }, "🖼️ PICTURE OF THE DAY", "media");
    } catch (e) { if (e.name !== 'AbortError') showError("Picture unavailable."); }
    busy = false;
  });
  if ($("trickOtd")) $("trickOtd").addEventListener("click", async () => {
    ...
    try {
      ...
      const d = await (await fetch(`https://api.wikimedia.org/feed/v1/wikipedia/en/onthisday/events/${mm}/${dd}`, { signal })).json();
      const ev = d.events[Math.floor(Math.random() * d.events.length)];
      const page = ev.pages && ev.pages[0];
      const full = page ? await fetchPage(page.title, signal) : { title: "On This Day", extract: ev.text, url: "https://en.wikipedia.org" };
      render({ ...full, extract: `<p style="text-align:center;"><i>${escapeHtml(ev.text)}</i></p>` + (full.extract || "") }, `📅 ON THIS DAY · ${ev.year}`, "media");
    } catch (e) { if (e.name !== 'AbortError') showError("History unavailable."); }
    busy = false;
  });

  /* Roulette — use setTimeout chain to prevent overlapping */
  function toggleRoulette() {
    const b = $("rouletteBtn"); if (!b) return;
    if (rouletteTimer) {
      clearTimeout(rouletteTimer); rouletteTimer = null;
      b.classList.remove("on"); b.textContent = "🔁 Auto Roulette";
      toast("ROULETTE OFF"); beep(300, .15);
    } else {
      rouletteNext();
      b.classList.add("on"); b.textContent = "⏸ Roulette · 5s";
      toast(currentCat === "adult" ? "🔞 FILTH ROULETTE — GOD HELP YOU" : "🔁 ROULETTE ON"); beep(1500, .15);
    }
  }
  async function rouletteNext() {
    if (!rouletteTimer && rouletteTimer !== 0) {
      /* First call — set a marker */
      rouletteTimer = -1;
    }
    await roll();
    if (rouletteTimer !== null) {
      rouletteTimer = setTimeout(rouletteNext, 5000);
    }
  }
  ...
  function updateLikeFavState() {
    ...
    /* Update FAB buttons too */
    if ($("fabLike")) { $("fabLike").classList.toggle("on", !!lk); $("fabLike").querySelector(".fab-icon").textContent = lk ? "❤️" : "🤍"; }
    if ($("fabSave")) {
      const saved = currentUser && userData && current && userData.saved.some(a => a.title === current.title);
      $("fabSave").classList.toggle("on", !!saved);
    }
  }
  function toggleList(type) {
    if (!current) return;
    if (!currentUser) { openPop("accountPop"); toast("⚠ SIGN IN TO SAVE / FAVORITE"); beep(200, .15); return; }
  ...
  if ($("saveBtn")) $("saveBtn").addEventListener("click", () => { if (!current) return; if (!currentUser) { openPop("accountPop"); toast("⚠ SIGN IN TO SAVE"); beep(200, .15); return; } const r = addToList("saved", current.title, current.url); if (r === null) toast("💾 ALREADY SAVED"); else if (r) { toast("💾 SAVED"); beep(1400); } });
  ...
  /* ============ KEYBOARD — FIXED: Space no longer hijacks scroll ============ */
  document.addEventListener("keydown", e => {
    if (e.repeat) return;

    /* Escape: close lightbox > popover > drawer */
    if (e.key === "Escape") {
    ...
    /* Space: Only trigger roll() if the cardBody is scrolled to the top
       or the document body is not scrollable (prevent scroll hijack) */
    if (e.code === "Space") {
      const cb = $("cardBody");
      if (cb) {
        const isScrollable = cb.scrollHeight > cb.clientHeight + 10;
        const isAtBottom = cb.scrollTop >= (cb.scrollHeight - cb.clientHeight - 20);
        if (isScrollable && !isAtBottom) {
          e.preventDefault();
          cb.scrollBy({ top: cb.clientHeight * 0.8, behavior: 'smooth' });
          return;
        }
      }
      e.preventDefault();
  ...
  (function setupTouchSwipe() {
    ...
    const MAX_TIME = 400;
    const MAX_Y_RATIO = 0.75; /* Horizontal dominance */
  ...
  /* Swipe hint visual feedback */
  function showSwipeHint(icon) {
    let hint = $("swipeHint");
    if (!hint) {
  ...
  function setupMobileFab() {
    if ($("fabNext")) $("fabNext").addEventListener("click", () => { roll(); beep(1200); });
    if ($("fabLike")) $("fabLike").addEventListener("click", () => toggleList("liked"));
    if ($("fabSave")) $("fabSave").addEventListener("click", () => {
      if (!current) return;
      if (!currentUser) { toast("⚠ SIGN IN TO SAVE"); beep(200, .15); return; }
      const r = addToList("saved", current.title, current.url);
      if (r === null) toast("💾 ALREADY SAVED"); else if (r) { toast("💾 SAVED"); beep(1400); }
      updateLikeFavState();
    });
    if ($("fabShare")) $("fabShare").addEventListener("click", () => {
      if (!current) return;
      if (navigator.share) navigator.share({ title: current.title, url: current.url }).catch(() => { });
      else copyHelper(current.url, "📡 LINK COPIED");
      beep(1100);
    });
    if ($("fabMenu")) $("fabMenu").addEventListener("click", () => {
      if (sidebarLeft && sidebarLeft.classList.contains("open")) closeDrawer();
      else openDrawer();
    });
  }
  function updateMobileFab() {
    /* Sync FAB like/save states with current article */
    updateLikeFavState();
  }
  setupMobileFab();

  /* ============ RIGHT SIDEBAR COLLAPSE (mobile) ============ */
  (function setupCollapsibleSidebar() {
    document.querySelectorAll(".sidebar-right .window").forEach(win => {
  ...
  /* ============ STARFIELD — Performance optimized ============ */
  (function () {
    ...
    function rs() {
      ...
      const maxStars = isMobile() ? 40 : Math.min(120, Math.floor(w * h / 16000));
      for (let i = 0; i < maxStars; i++) {
        starArr.push({
          x: Math.random() * w, y: Math.random() * h,
          r: Math.random() * 1.6 + .5,
          p: Math.random() * Math.PI * 2,
          s: .006 + Math.random() * .015,
          blue: Math.random() < .25
        });
      }
    }
    ...
    addEventListener("resize", () => {
      ...
      resizeStarsTimer = setTimeout(rs, 200);
    });
    rs();
    ...
    function loop() {
      ...
      cx.globalAlpha = 1;
      if (tick % 460 === 0 && Math.random() < .7) {
        shots.push({ x: Math.random() * w * .8, y: Math.random() * h * .3, vx: 5 + Math.random() * 3, vy: 1.5 + Math.random() * 1.5, life: 40 });
      }
    ...
    /* Page Visibility API — pause starfield when tab is hidden */
  ...
  function applyTheme(t) {
    ...
    const btn = $("themeBtn");
    /* Show ☀️ in dark mode (to indicate switch-to-light) and 🌙 in light mode */
    if (btn) btn.textContent = theme === 'dark' ? '☀️' : '🌙';
    /* Update meta theme-color for mobile browsers */
    const mc = document.querySelector('meta[name="theme-color"]');
    if (mc) mc.setAttribute('content', theme === 'dark' ? '#0b1220' : '#f4f6f8');
  }
